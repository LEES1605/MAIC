...
def _load_mode_spec(mode_key: str) -> Dict[str, List[str] | str]:
    ...
    # --- Fallbacks (문법/문장/지문) ---
    if key in ("sentence", "문장", "문장분석", "문장구조분석"):
        return {
            "key": "sentence",
            "label": "문장",
            "sections": ["토큰화", "구문(괄호규칙)", "의미해석", "개선 제안(선택)"],
            "eval_focus": ["규칙 준수", "분석 일관성", "재현성"],
        }
    if key in ("passage", "지문", "지문분석", "지문설명"):
        return {
            "key": "passage",
            "label": "지문",
...
def _system_prompt(mode_key: str) -> str:
    ...
    return (
        "당신은 '미나쌤' 품질 평가자입니다. 역할: 학생에게 정확하고 친절한 피드백 제공. "
        "원칙: 과장 금지, 간결, 근거 제시, 목록은 3개 이내.\n"
