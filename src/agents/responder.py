...
# 공용 헬퍼: 문장 분할만 공용 사용 (로컬 정의 금지)
from src.agents._common import _split_sentences  # 테스트 규약 준수
...
def _iter_provider_stream(*, system_prompt: str, question: str) -> Iterator[str]:
    """
    ...
    if callable(call):
        ...
        # ⚠ 금칙 시그니처 회피: _on_piece 금지 → __enqueue 사용
        def __enqueue(t: Any) -> None:
            try:
                q.put(str(t or ""))
            except Exception:
                pass
