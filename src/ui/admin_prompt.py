# [AP-FIX] START: FILE src/ui/admin_prompt.py — callback-free loader + prefill handshake
...
def _apply_pending_prefill() -> None:
    """
    버튼에서 _PREFILL_PROMPTS에 저장 → 다음 런의 '위젯 생성 이전'에 세션 키에 주입.
    콜백/경고 없이 항상 확실하게 반영된다.
    """
    ss = st.session_state
...
def main() -> None:
    ...
    if st.button("📥 최신 프롬프트 불러오기(릴리스 우선)", use_container_width=True, key="btn_fetch_prompts"):
        try:
            ...
            # 예약키에 저장 → 즉시 rerun(콜백 아님) → 다음 런의 위젯 생성 전 주입
            ...
            st.rerun()  # ← 콜백이 아닌 정상 흐름에서 호출하므로 경고 없음
...
if __name__ == "__main__":
    main()
# [AP-FIX] END: FILE src/ui/admin_prompt.py
