...
def render_index_status_panel(
    *,
    ...
    - '파일 수 갱신' 버튼: 로컬 persist 파일 개수만 새로 고침
    """
    ...
    rel = None
    chosen_tag = None
    try_tags: list[str] = list(tag_candidates) + ["latest"]
    for t in try_tags:
        try:
    ...
    with col1:
        if st.button("🔄 Release에서 최신 인덱스 복원", use_container_width=True):
            try:
                ...
                # res는 RestoreLog (tag/release_id/detail/...)
                if not isinstance(res, RestoreLog):
                    # 혹시 다른 타입일 상황(과거 코드 대비)에도 최대한 정보 추출
                    rtag = getattr(res, "tag", None) or getattr(res, "tag_name", None)
