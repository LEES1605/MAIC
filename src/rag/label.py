...
- decide_label: 히트의 파일명/확장자 규칙으로 라벨을 결정.
  ...
  * 그 외(히트가 1개 이상) → [문법책]
  * 히트가 없을 때만 → [AI지식]
- make_source_chip: UI 칩 표시용 문자열 생성. 예) "[문법책] · Advanced Grammar in Use"
"""
...
__all__ = ["search_hits", "decide_label", "make_source_chip"]
...
def make_source_chip(
    hits: Iterable[Dict[str, Any]] | None,
    label: str,
    *,
    max_title_len: int = 28,
) -> str:
    """
    UI 칩 표시용 문자열:
      예) "[문법책] · Advanced Grammar in Use"
    - 히트가 없거나 제목이 없으면 라벨만 반환.
    """
    items = list(hits or [])
    if not items:
        return label
    ...
    return f"{label} · { _shorten(name, max_title_len) }"
# =============================== [01] RAG LABELER — END ===============================
