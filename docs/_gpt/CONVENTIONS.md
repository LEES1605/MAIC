...
- **운영 원칙**: 사용자 요청에 따라 **Cursor 프로그램을 사용하여 로컬에서도 개발을 지원한다**(CI/레포 반영과 병행).
...
5. **CI 우선**: Cursor 프로그램 사용 시 로컬 실행을 허용하되, **CI/PR 기반**으로 재현·검증을 병행한다. :contentReference[oaicite:4]{index=4}
...
**금지/예외**  
- **금지**: 존재하지 않는 파일/경로/심볼에 대한 패치 지시. (Cursor 프로그램 사용 시 로컬 실행 지시는 허용) :contentReference[oaicite:8]{index=8}  
