...
2. **PR‑S1** — 타이핑 핸드셰이크 & 즉시 스트리밍(타이핑 ≤0.2s, 첫 토큰 ≤0.2s, 출처칩 지연 갱신).  
3. **PR‑H1** — 헤더 3단계 표기 & 최신 릴리스 인지(🟩=최신 복원).  
4. **PR‑RAG1** — Top‑K **프롬프트 명시 주입**, **Strict 게이트**.  
5. **PR‑A2** — Prompt Orchestrator(관용 파서·Auto‑Repair·Clarify≤1).  
6. **PR‑A3‑DA** — 문법 더블에이전트(PT→MN), 문장/지문=MN 단일, 맥락 유지·Clarify.  
7. **PR‑A3‑DA‑SUM** — **요약/비교(3+1줄)** 자동 생성, 상충 감지/권고문 포함.  
8. **PR‑QF4‑SSOT** — **중복 제거·정합**(persist/prepared/layout 단일화).  
9. **PR‑I3** — 모델 라우팅(기본/폴백, 폴백 로그/배지).  
10. **PR‑C3** — Playwright E2E 스모크(2건).  
11. **PR‑C3‑Plus** — **E2E 확장 원칙** + Top‑5 고위험 시나리오.  
12. **PR‑S2** — Supabase Auth(+RBAC), 관리자 승인 가입.  
13. **PR‑S2‑Legal** — Privacy/Retention 문서, 동의 UI/로그, DSR, 가명화 ETL.  
14. **PR‑U1/U2** — 디자인 토큰/컴포넌트화.  
15. **PR‑I4‑Image** — 이미지 업로드→세션 한정 OCR/비전(영어 지문/문제), PDF 제외.
